...
public interface MessageMapper extends BaseMapper<Message> {
    ...
    @Select("""
            ...
            WHERE (mrs.is_read IS NULL OR mrs.is_read = 0)
            """)
    ...
    @Select("""
            ...
            JOIN message_target_group mtg ON mtg.message_id = m.message_id
            JOIN user_group ug ON ug.group_id = mtg.group_id
            WHERE m.message_id = #{messageId} AND ug.user_id = #{userId}
            """)
